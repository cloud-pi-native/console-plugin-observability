...
export class ObservabilityRepoManager {
  ...
  private async findOrCreateValuesFile(project: ProjectSchema): Promise<ObservabilityData> {
    const yamlData = `
    global:
      tenants: []
    `
    ...
    try {
      ...
      return yaml.load(Buffer.from(yamlData, 'base64').toString('utf-8')) as ObservabilityData
    }
  }
  ...
  public async updateProjectConfig(project: Project, projectValue: ObservabilityProject): Promise<string> {
    ...
    const yamlFile = await this.findOrCreateValuesFile(gitlabRepo)
